...
$stmt = $con->prepare("UPDATE tblposts SET PostImage='$imgnewfile' where id='$postid'");
    $stmt->bind_param("si", $imgnewfile, $postid);
$stmt->execute();
if($stmt)
{
...
$stmt = $con -> prepare('select PostImage,PostTitle from tblposts where id=? and Is_Active=1');
$postid=intval($_GET['pid']);
$stmt -> bind_param('i', $postid);
$stmt -> execute();
$stmt -> store_result();
$stmt -> bind_result($PostImage,$PostTitle);
$stmt->fetch();
?>
                        <div class="row">
...
<input type="text" class="form-control" id="posttitle" value="<?php echo $PostTitle;?>" name="posttitle"  readonly>
</div>
