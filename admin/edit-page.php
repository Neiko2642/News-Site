...
{
$pagetype=intval($_GET['pid']);
$pagetitle=$_POST['pagetitle'];
$pagedetails=$_POST['pagedescription'];

$stmt = $con->prepare("UPDATE tblpages SET PageTitle='$pagetitle',Description='$pagedetails' where id='$pagetype'");
    $stmt->bind_param("ssi", $pagetitle,$pagedetails,$pagetype);
$stmt->execute();
if($stmt)
{
...
<html lang="en">
    <head>
        ...
        <!-- App title -->
        <title>NeikoPanel | Edit Page</title>
...
<?php
$stmt = mysqli_prepare($con, "select id,PageTitle,Description from tblpages where id=?");
$stmt->bind_param("i", $pagetype);
$pagetype=intval($_GET['pid']);
$stmt->execute();
$stmt->bind_result($id, $PageTitle, $description);
$stmt->fetch();
?>
...
<input type="text" class="form-control" id="pagetitle" name="pagetitle" value="<?php echo htmlentities($PageTitle);?>"  required>
</div>
...
<textarea class="summernote" name="pagedescription"  required><?php echo htmlentities($description);?></textarea>
</div>
</div>
</div>
